...
def _create_invoice(obj, cr, uid, data, context=None):
    ...
    for pick in pickings:
        obj_pick= picking_obj.browse(cr,uid,pick)
        if obj_pick.invoice_state != '2binvoiced':
            picking_obj.write(cr,uid,pick,{'invoice_state': '2binvoiced'})
...
class wizard_reinvoice(wizard.interface):

#    form1 = '''<?xml version="1.0"?>
#    <form string="Reinvoice">
#        <label string="Create Invoice with the selected Stock-pickings?" width="100"/>
#     </form > '''
#    form1_fields = {} 
    ...
    states = {
            'init': {
                     'actions': [_get_type],
                     'result': {'type': 'form','arch': invoice_form,'fields': invoice_fields,'state': [('end', 'Cancel'),('create_invoice', 'Create invoice')]}
                        },
            'create_invoice': {
            'actions': [],
            'result': {
...
wizard_reinvoice ("reinvoice.wizard")
