...
class Aeroo_report(report_sxw):
    ...
    def create_aeroo_report(self, cr, uid, ids, data, report_xml, context=None, output='odt'):
        ...
        module_info = load_information_from_description_file('trey_report_aeroo')
	...
	#version = '1.0'
        ...
        if output in ('pdf-command') :
            import random
            import commands

            if company_id :
                command = pool.get('res.company').browse(cr, uid, company_id, context=context).aero_pdf_convert_command
            else :
                company_ids = pool.get('res.company').search(cr, uid, [] )                
                command = pool.get('res.company').browse(cr, uid, company_ids[0], context=context).aero_pdf_convert_command


            filename = '/tmp/openerp_aeroo_%s' % (random.randint(0, 99999))
            filename_odt = '%s.odt' % (filename)
            filename_pdf = '%s.pdf' % (filename)
            with open(filename_odt,'w') as f:
                f.write(data)
            f.close()


            logging.getLogger('trey_aeroo_report').info("Comando de impresion: %s --outdir /tmp/ %s"  % (command, filename_odt))
            stat, out = commands.getstatusoutput('%s --outdir /tmp/ %s'  % (command, filename_odt))
            logging.getLogger('trey_aeroo_report').info("Stat: %s"  % (stat))
            logging.getLogger('trey_aeroo_report').info("Out: %s"  % (out))


            f = open(filename_pdf,'r')
            data = f.read()
            f.close()
            output = 'pdf'

        elif (output!=report_xml.in_format[3:] or self.oo_subreports):
            if aeroo_ooo and DC:
                try:
                    DC.putDocument(data)
    ...
    def create(self, cr, uid, ids, data, context=None):
        ...
        if report_type in ['sxw','odt']:
            ...
            if report_xml.out_format.code in ['oo-pdf']:
                fnct = self.create_source_pdf
            elif report_xml.out_format.code in ['oo-odt','oo-ods','oo-doc','oo-xls','oo-csv','genshi-raw','oo-pdf-command']:
                fnct = self.create_source_odt
            else:
