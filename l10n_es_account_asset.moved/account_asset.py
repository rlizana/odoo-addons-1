...
class account_asset_asset(osv.osv):
    ...
    def _get_last_depreciation_date(self, cr, uid, ids, context=None):
        ...
        """
        cr.execute("""
            SELECT a.id as id, COALESCE(MAX(l.date),a.deprec_start_date) AS date
            FROM account_asset_asset a
            LEFT JOIN account_move_line l ON (l.asset_id = a.id)
            WHERE a.id IN %s
            GROUP BY a.id, a.deprec_start_date """, (tuple(ids),))
    ...
    _columns = {
        ...
        'deprec_start_date': fields.date('Depreciation starting Date', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        ...
        'compute_at_end_period': fields.boolean('Compute at the end of period'),
...
class account_asset_depreciation_line(osv.osv):
    ...
    def create_move(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            reference = str(depreciation_date) + " " + line.asset_id.name 
