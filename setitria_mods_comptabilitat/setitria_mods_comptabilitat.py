                ...
                for grupo in grupoTotal['groups']:  # per cada grup (agrupacio de rebuts per dia)
                    for linea in grupo['lineas']:   # per cada rebut
                        ...
                        if partner:
                            ...
                            raise osv.except_osv('Error', 'Partner not found containing %s Reference number.' % linea['codi_ref_int'])
                        
                        ids = False
                        pline = False
                        pline_ids = pay_line.search(cr, uid, [('ml_inv_ref', '=', linea['codi_dev'])])
                        for line in pline_ids:
                            pline = pay_line.browse(cr,uid,line)
                            if pline.name == linea['codi_ref_int']:
                                break
                            else:
                                pline = False
                        
                        if pline and pline.ml_inv_ref:
                            if pline.ml_inv_ref.number == linea['codi_dev']:
                        ...
                        if ids:
                            ...
                            if id_linea:
                                ...
                                if id_reconcile:
                                    ...
                                    apunt.write(cr,uid,ids_mov,{'reconcile_id':id_reconcile},context)
