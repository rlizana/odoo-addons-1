...
class c2c_export_pricelist(osv.osv_memory):
    ...
    def export_data(self,cr,uid,ids,context=None):
        ...
        keys=['Product Code','EAN13', 'Supplier', 'Supplier Delay', 'Supplier Min. Qty', 'Supplier Code', 'Supplier Name', 'Quantity', 'Price']
        #Encode keys        
        ...
        prod_ids = product_obj.search(cr, uid, [('default_code', '!=', False), ('default_code', '!=', '')])
        if not prod_ids:
            prod_ids = product_obj.search(cr, uid, [('default_code', 'like', ''),('ean13', '!=', False)])
        if prod_ids == []:
            raise osv.except_osv('Error', 'We cannot find any product !')
            ...
            if prod.seller_ids:
               for seller in prod.seller_ids:
                    for supplier in seller.pricelist_ids:
                        row = []
                        row.append(prod.default_code.encode("utf-8") or ' ')
                        row.append(prod.ean13 or ' ')
                        row.append(seller.name.name.encode("utf-8") or '')
                        row.append(seller.delay or ' ')
                        row.append(seller.qty or ' ')
                        if seller.product_name:
                            row.append(seller.product_name.encode("utf-8") or ' ')
                        else:
                            row.append(' ')
                        if seller.product_code: 
                            row.append(seller.product_code.encode("utf-8") or ' ')
                        else:
                            row.append(' ')
