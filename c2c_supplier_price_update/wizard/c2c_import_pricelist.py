        ...
        for wiz in self.browse(cr, uid, ids, context):
            ...
            if file_type[1] == 'xls':
                ...
                if xlrd:
                    ...
                    file.close()
                    #Read xls file.
                    book = xlrd.open_workbook(fp_name, encoding_override='cp1252')
            ...
            for i in range(len(reader_info)):
                ...
                if values['Product Code']:
                    prod_ids = prod_obj.search(cr, uid, [('default_code', '=', values['Product Code'])])
                if values['EAN13'] and prod_ids == []:
                    prod_ids = prod_obj.search(cr, uid, [('ean13', '=', values['EAN13'])])
                if prod_ids == []:
                    raise osv.except_osv('User error', 'There is no code to identify a product !')
                
                if prod_ids and values['Supplier'] != '-':
                    for prod in prod_obj.browse(cr, uid, prod_ids):
                        if not prod.default_code:
                            prod_obj.write(cr, uid, prod.id, {'default_code': values['Product Code']})
                        elif not prod.ean13:
                            prod_obj.write(cr, uid, prod.id, {'ean13': values['EAN13']})
