...
# fin Copia de Res Parter.crm v5
...
#        return self.pool.get('res.users').browse(cr, uid, uid, context).address_id.id
        result = self.pool.get('res.users').browse(cr, uid, uid, context).company_id.partner_id.address_id.id
        return True
    ...
    def __history(self, cr, uid, cases, keyword, history=False, email=False, context={}):
        for case in cases:
            ...
            if history and case.description:
                ...
                data['email'] = email or \
                        (case.user_id and \
                            case.user_id.user_email) or False
