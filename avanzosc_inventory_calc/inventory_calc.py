...
class stock_inventory_line(osv.osv):
    ...
    _columns = {
        'cost_price': fields.float('Cost Price', size=16)
        }
    
stock_inventory_line()


class stock_fill_inventory_cost(osv.osv_memory):
    
    _name = "stock.fill.inventory.cost"
    _description = "Import Inventory Cost"
    _columns = {
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        ...
        if context is None:
            context = {}
        super(stock_fill_inventory_cost, self).view_init(cr, uid, fields_list, context=context)
    ...
    def fill_inventory_cost(self, cr, uid, ids, context=None):
        ...
        for location in location_ids:
            ...
            for move in move_obj.browse(cr, uid, move_ids, context=context):
                ...
                product_reg = product_obj.browse(cr,uid,prod_id)
                datas[(prod_id)] = {'product_id': prod_id, 'location_id': location, 'product_qty': qty, 'product_uom': move.product_id.uom_id.id, 'cost_price':product_reg.standard_price}
