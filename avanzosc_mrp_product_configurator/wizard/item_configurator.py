...
class mrp_bom_configurator(osv.osv_memory):
    ...
    _columns = {
            'mrp_production': fields.many2one('mrp.production', 'Production Order', readonly=True),
            ...
            'installer_id': fields.many2one('res.partner', 'Installer', readonly=True),
            'technician_id': fields.many2one('res.partner.contact', 'Technician'),
            'customer_id': fields.many2one('res.partner', 'Customer', readonly=True),
            'customer_addr_id': fields.many2one('res.partner.address', 'Customer Address', readonly=True),
    ...
    def next(self, cr, uid, ids, context=None):
        wizard = {'type': 'ir.actions.act_window_close'}
