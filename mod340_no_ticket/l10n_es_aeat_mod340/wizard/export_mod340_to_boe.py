...
#            elif invoice_issued.invoice_id.journal_id.invoice_key != '':
#                text +=self._formatString(invoice_issued.invoice_id.journal_id.invoice_key, 1)               # Clave de operación
            else: text += ' '
    ...
    def _get_formated_invoice_received(self, cr, uid, report, invoice_received):
        ...
        for tax_line in invoice_received.tax_line_ids:
            ...
            if invoice_received.invoice_id.origin_invoices_ids:
                text +='D'
            elif len(invoice_received.tax_line_ids) > 1: text += 'C'              # Clave de operación
#            elif invoice_received.invoice_id.journal_id.invoice_key != '':
#                text +=self._formatString(invoice_received.invoice_id.journal_id.invoice_key, 1)
            else: text += ' '
