...
#                    if invoice.journal_id.invoice_key not in  ('J','', False):
#                        raise osv.except_osv(_('La siguiente clave de factura no esta implementada:'), invoice.journal_id.invoice_key)
                    
                    if invoice.partner_id.vat:
                        nif = invoice.partner_id.vat and re.match(r"([A-Z]{0,2})(.*)", invoice.partner_id.vat).groups()[1]
                        values.update({'partner_vat': nif})
#                    elif not invoice.partner_id.vat and invoice.journal_id.invoice_key != 'J':
#                        raise osv.except_osv(_('La siguiente empresa no tiene asignado nif:'), invoice.partner_id.name)
                    ...
                    for tax_line in invoice.tax_line:
                        if tax_line.name.find('IRPF') == -1: # Remove IRPF from Mod340
                            ...
                            if not account_tax:
                                raise osv.except_osv(_('IVA mal configurado en la factura:'), invoice.number)                            
