...
class product_pack( osv.osv ):
    ...
    def _check_recursion(self, cr, uid, ids, context=None):
        for obj_list in self.browse(cr, uid, ids, context=context):
            if obj_list.parent_product_id:
                main_parent = obj_list.parent_product_id
                product= obj_list.product_id
                if main_parent.id == product.id:
                    return False
        return True
    ...
    _constraints = [
        (_check_recursion, 'Error ! You cannot assign the Product Pack as component of the same Produck Pack!', ['parent_product_id'])
    ]
